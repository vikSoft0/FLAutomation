...
namespace FLAutomation.ComponentHelper
{
    ...
    {
        private const int timeSpanInSecond = 60;
        ...
        public static void TakeScreenShot(string filename = "Screen")
        {
            ...
            screen.SaveAsFile(filename); Logger.Info(" ScreenShot Taken : " + filename);
        }
        ...
        public static IWebElement WaitAndFindElementInPage(By locator)
        {
            try
            {
                ObjectRepository.Driver.Manage().Timeouts().ImplicitWait = (TimeSpan.FromSeconds(1));
                Logger.Info(" Setting the Explicit wait to 1 sec ");
                var wait = GetWebdriverWait(TimeSpan.FromSeconds(timeSpanInSecond));
                var flag = wait.Until(WaitForWebElementInPageFunc(locator));
                Logger.Info(" Setting the Explicit wait Configured value ");
                ObjectRepository.Driver.Manage().Timeouts().ImplicitWait = (TimeSpan.FromSeconds(ObjectRepository.Config.GetElementLoadTimeOut()));
                return flag;
